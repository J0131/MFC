...
UINT RXThread(LPVOID arg)
{
	...
	CUDPServer_thdDlg *pDlg = (CUDPServer_thdDlg*)pArg->pDlg; // CTCPServer_thdDlg*형 pDlg에 (CTCPServer_thdDlg*)pArg->pDlg를 저장
	while (pArg->Thread_run) { // Thread_run이 1일동안 반복
		...
		while (pos != NULL) // pos 가 NULL이 아닐동안 반복
		{
			...
			message += str;
			message += "\n";
			pDlg->m_rx_edit.SetWindowText(message); // message를 m_rx_edit에 출력
...
UINT TXThread(LPVOID arg)
{
	...
	while (pArg->Thread_run) // Thread_run이 1일동안 반복
	{
		...
		while (pos != NULL) { // pos 가 NULL이 아닐동안 반복
			...
			pDlg->m_pDataSocket->SendToEx(str, (str.GetLength()) * sizeof(TCHAR),port,addr,0); // str을 sendtoex함수를통해 해당포트와 주소의 소켓에 전송
