...
int certify_checksum(TCHAR* t,int chk);
CString re,message3,message4;
...
CARQServer_thdDlg::CARQServer_thdDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(IDD_ARQSERVER_THD_DIALOG, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CARQServer_thdDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
...
BEGIN_MESSAGE_MAP(CARQServer_thdDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_SEND, &CARQServer_thdDlg::OnBnClickedSend)
	ON_BN_CLICKED(IDC_CLOSE, &CARQServer_thdDlg::OnBnClickedClose)
END_MESSAGE_MAP()


// CARQServer_thdDlg 메시지 처리기
UINT RXThread(LPVOID arg)
{
	...
	CARQServer_thdDlg *pDlg = (CARQServer_thdDlg*)pArg->pDlg; // CARQClient_thdDlg*형 pDlg에 (CARQClient_thdDlg *)pArg->pDlg를 저장
	UINT port=8001; // 포트 선언
	...
	while (pArg->Thread_run) { // Thread_run이 1일동안 반복
		...
		while (pos != NULL) // pos 가 NULL이 아닐동안 반복
		{
			...
			{ // ack 메세지를 받았다는것을 출
				pDlg->m_edit2.GetWindowText(message); // m_rx_edit를 읽어 message에 저장 
				message += str;
				con2.Format(_T("Receive Acknowledge Frame(%d): %s ==> [seq_num : %d ack_num : %d checksum : %d ] \r\n"), frame.seq_num, str, frame.seq_num, frame.ack_num, certify_checksum(frame.p_buffer, frame.checksum));
				pDlg->m_edit2.GetWindowText(con_2);
				con_2 += con2;
				con_2 += "\n";
				ack = frame.ack_num;
			...
			if (certify_checksum(frame.p_buffer, frame.checksum) == 0 && send_flag != 1) // 체크섬을 확인함=> 중복메세지인지 확인, send_flag가 1이면 => ack메세지를 보내주어야함을뜻함
			{
			...
			/*pDlg->m_edit2.SetWindowText(message); // message를 m_rx_edit에 출력
			pDlg->m_edit2.LineScroll(pDlg->m_edit2.GetLineCount());

			pDlg->m_edit3.SetWindowText(re); // message를 m_rx_edit에 출력*/
...
UINT TXThread(LPVOID arg)
{
	...
	while (pArg->Thread_run) // Thread_run이 1일동안 반복
	{
		...
		while (pos != NULL) { // pos 가 NULL이 아닐동안 반복
			current_pos = pos; // pos를 current_pos에 넣어줌
			pDlg->m_pDataSocket->GetPeerName(addr, port); // 연결된 소켓의 addr와 port정보를 저장

			tx_cs.Lock(); // 임계구역 들어가기
			Frame frame = flist->GetNext(pos); // flist의 pos위치 원소를 frame에저장후 pos증가
			TCHAR* message1 = frame.p_buffer;
			...
			message += "\n";
													//message += "\r\n";
			pDlg->m_edit3.SetWindowTextW(message); // message를 m_tx_edit에 출력*/
			...
			while (ack != frame.seq_num) // 이때 프레임이 전송되었으나 ack메세지를 받지못했다면 재전송하기위한 프로세스
			{
				...
				while (1) // 무한반복
				{
					...
					if ((clock() - start) / CLOCKS_PER_SEC > 5.0f) // 프레임전송후 ack메세지가 5초동안 오지않으면 반복문 나가고 재전송
						break;
				}
			} 
...
BOOL CARQServer_thdDlg::OnInitDialog()
{
	...
	if (m_pDataSocket->Create(8000, SOCK_DGRAM)) // datasocket udp형식으로 생성
	{
...
void CARQServer_thdDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CARQServer_thdDlg::OnPaint()
{
	if (IsIconic())
	{
...
//  이 함수를 호출합니다.
HCURSOR CARQServer_thdDlg::OnQueryDragIcon()
{
...
void CARQServer_thdDlg::ProcessReceive(CDataSocket* pSocket, int nErrorCode)
{
	TCHAR pBuf[1024 + 1];
	CString strData;
	Frame f;
	...
	UINT port = 8000; // addr선언
	CString addr = _T("127.0.0.1"); // port선언
...
void CARQServer_thdDlg::ProcessClose(CDataSocket* pSocket, int nErrorCode)
{
	pSocket->Close();
...
void CARQServer_thdDlg::OnBnClickedSend()
{
	...
	else
	{
		...
		if (message.GetLength() <= 16) // message의 길이가 16이하일때 
		{
			...
			checksum = certify_checksum(f.p_buffer, chk); // p_buffer의 체크섬을 구해 checksum에 저장
			f.checksum = checksum;
			f.ack_num = ack_num;
		...
		else if (message.GetLength() > 16)  // message의 길이가 16보다 클때
		{
			...
			for (int i = 0; i < (message.GetLength() / 16) + 1; i++)
			{
				...
				checksum = certify_checksum(f1.p_buffer, chk);  // f1의 p_buffer의 체크섬을 구하여 checksum에저장
...
void CARQServer_thdDlg::OnBnClickedClose()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
